...
def send_transaction(contract: Contract, account: Any, function_name: str, *args, **tx_params) -> str:
    ...
    print(f"  {function_name}() | TX: {tx_hash.hex()[:16]}... | Block: {receipt['blockNumber']}")
    return tx_hash.hex()
...
def scenario_happy_path(contract: Contract) -> None:
    ...
    try:
        response = api_call("POST", "/api/v1/agreements", json=agreement_data)
    ...
    # Step 2: Submit agreement via API
    print("\nStep 2: POST /agreements/{id}/submit (PENDING_FUNDING)")
    try:
        api_call("POST", f"/api/v1/agreements/{agreement_id}/submit")
        print(f"  Submitted agreement {agreement_id}")
    except Exception as e:
        print(f"  Failed to submit agreement: {e}")
        return
    
    # Step 3: Create agreement on-chain
    print("\nStep 3: createAgreement() on smart contract")
    ...
    print("\nWaiting 5 seconds for worker to process AgreementCreated event...")
    time.sleep(10)
    
    # Step 4: Fund agreement on-chain
    print("\nStep 4: fund() on smart contract")
    send_transaction(
        contract, PAYER, "fund",
        agreement_id_bytes,
    ...
    print("\nWaiting 5 seconds for worker to process PaymentFunded event...")
    time.sleep(10)
    
    # Step 5: Release payment
    print("\nStep 5: release() on smart contract")
    send_transaction(contract, PAYER, "release", agreement_id_bytes)
    
    print("\nWaiting 5 seconds for worker to process PaymentReleased event...")
    time.sleep(10)
...
def scenario_dispute_flow(contract: Contract) -> None:
    """Test dispute flow: Create → Submit → createAgreement → fund → dispute → arbitrator release."""
